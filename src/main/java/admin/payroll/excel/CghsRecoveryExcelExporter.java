...
public class CghsRecoveryExcelExporter {
	private XSSFWorkbook workbook;
	private XSSFSheet sheet;
	private List<PaybillAndOtherEntity> listUsers;

	public CghsRecoveryExcelExporter(List<PaybillAndOtherEntity> listUsers) {
	...
	private void writeHeaderLine() {
		...
		createCell(row, 2, "Basic(Rupees)", style);
		createCell(row, 3, "Amount(Rupees)", style);
	...
	private void writeDataLines() {
		...
		for (PaybillAndOtherEntity user : listUsers) {
			...
			createCell(row, columnCount++, user.getCghs(), style);
		   // createCell(row, columnCount++, user.getSsLic(), style);
